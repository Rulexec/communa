...
var status = exports.status = {
  ...
  working: [{ // bikeprofile
    ...
    description: '42км',
    updates: [
      '<p><i>2013-07-15</i> — страницы для отдельных треков.</p>' +
      '<p>Пожалел, что не делал в git отдельную ветку для каждой отдельной фичи. Из-за чего запутался с недоделанным kml и отдельными страницами.</p>',

      '<i>2013-07-13</i> — kml почти добавляется, осталось лишь на сервере валидировать и сохранять пути.',
      '<i>2013-07-12</i> — треки теперь хранятся в БД. Настроено кеширование запросов треков. Минорные изменения отображения списка путей (теперь новые вверху, дата перед названием, цвет названия).',
